...
def run(src_path, res_path, batch_sz):
    ...
    # Helper function to find the line beginnings for each article by Regex
    def find_record_beginning(line):
        ...
        return new_record_check

    # Find news article record beginning in file, reading one line at a time in file
    beginning_lines_nr_ls = []
    with open(src_path, 'r') as f:
        for i, line in tqdm(enumerate(f)):
            if find_record_beginning(line):
                beginning_lines_nr_ls += [i]

    # Loop through all lines to start extracting each element and write to JSON
    batch_nr = 1
    parsed_dct = {}
    __parsed_string = ''
    ...
    # Helper func to parse string into a Python dict, returns None if failed validation checks
    ...
    # For every line beginning found up to the next line nr (exclusive), read the lines in file
    for i, (curr_line_nr, ubound_line_nr) in tqdm(enumerate(curr_next_lines_tup_ls)):
        __parsed_string = ''
        with open(src_path, 'r') as f:
            for i, line in enumerate(f):
                # If in range, capture parsed string by concatenation
                if curr_line_nr <= i < ubound_line_nr:
                    __parsed_string += line

                # Get out of loop after passing the upper bound
                elif i >= ubound_line_nr:
                    break
