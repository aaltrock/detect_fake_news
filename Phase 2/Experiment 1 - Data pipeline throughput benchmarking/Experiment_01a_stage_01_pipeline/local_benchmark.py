"""
01 Benchmark throughput local direct runner for stage 1 of the data pipeline
"""
...
def run(argv=None, save_main_session=True):
    ...
    for sample_sz in [1250, 2500, 5000, 7500, 10000]:
        for i in range(repeat_nr):
            ...
            bigquery_output_table = 'src_fake_news_' + str(sample_sz)
            # param_str = '--region europe-west2 --input=' + parquet_file_nm + ' --output=gs://src_fake_news_bs --runner=DataflowRunner --project=fake-news-bs-detector --staging_location=gs://src_fake_news_bs/staging --temp_location=./tmp/ --template_location=./template1 --requirements_file=./requirements_01_gcp_bucket_to_bigquery_dataflow.txt --source_bucket_name=src_fake_news_bs --source_bucket_base_path=to_add --processed_bucket_success=added --bigquery_output_dataset=fake_news --bigquery_output_table=' + bigquery_output_table + ' --json_key_path=' + json_key_path
            # param_str = '--region europe-west2 --input=' + parquet_file_nm + ' --output=gs://src_fake_news_bs --runner=DirectRunner --project=fake-news-bs-detector --staging_location=gs://src_fake_news_bs/staging --temp_location=./tmp/ --requirements_file=./requirements_01_gcp_bucket_to_bigquery_dataflow.txt --source_bucket_name=src_fake_news_bs --source_bucket_base_path=to_add --processed_bucket_success=added --bigquery_output_dataset=fake_news --bigquery_output_table=' + bigquery_output_table + ' --json_key_path=' + json_key_path
            param_str = '--region europe-west2 --input=' + parquet_file_nm + ' --output=gs://src_fake_news_bs --runner=DirectRunner --project=fake-news-bs-detector --staging_location=gs://src_fake_news_bs/staging --temp_location=./tmp/ --template_location=./src_fake_news_bs/template1 --requirements_file=./requirements_01_gcp_bucket_to_bigquery_dataflow.txt --source_bucket_name=src_fake_news_bs --source_bucket_base_path=to_add --processed_bucket_success=added --bigquery_output_dataset=fake_news --bigquery_output_table=' + bigquery_output_table + ' --json_key_path=' + json_key_path
    ...
    benchmark_res_df.to_csv('direct_runner_benchmark_res_df.csv', index=False)
